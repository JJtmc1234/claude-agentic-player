...
def main():
    ...
    p = spec.placements
    check("drill pos", (-14.0, 51.0), (p[0].x, p[0].y))
    check("drill is burner-mining-drill", 'burner-mining-drill', p[0].item)
    check("furnace pos", (-14.0, 53.0), (p[1].x, p[1].y))
    check("furnace is stone-furnace", 'stone-furnace', p[1].item)
    check("inserter pos", (-13.5, 54.5), (p[2].x, p[2].y))
    check("inserter dir=NORTH(0)", 0, p[2].direction)
    check("chest pos", (-13.5, 55.5), (p[3].x, p[3].y))
