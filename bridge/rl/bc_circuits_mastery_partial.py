"""BC pretrain on the 10-action PARTIAL mastery demo (multi-product cable+circuit).

The agent gets a skeleton (2 asms + 5 bridging inserters + 3 anchor belts) and
must fill in the input/output/cable-conveyor belts itself. Mod 0.10.2 supplies
tiered partial rewards (cables in transit, inserter bridging, etc).
"""
...
from rl.demonstration_circuits_16x16_mastery_partial import as_actions
...
def configure_circuits_mastery():
    """Mastery: raw plates only, agent must make cables themselves."""
    with RconClient() as r:
        spec = {
            'recipe_name': 'electronic-circuit',  # primary asm recipe
            'output_item': 'electronic-circuit',
            'target_output': 20,
            'recipe_options': ['copper-cable', 'electronic-circuit'],
            'input_items': [
                {'name': 'iron-plate', 'count': 40},
                {'name': 'copper-plate', 'count': 80},  # mastery: only plates, no cables
            ],
            'sim_max_ticks': 3600,
        }
...
def main() -> int:
    ...
    p.add_argument('--save', default='checkpoints/maskppo_bc_circ16_mastery_partial.zip')
    args = p.parse_args()
    ...
    if not configure_circuits_mastery():
        print('[bc-circ16-mp] failed to set mastery task')
        return 1

    raw_env = MaskableFactorioArenaEnv()
    env = ActionMasker(raw_env, mask_fn)
    demo = list(as_actions())
    print(f'[bc-circ16-mp] partial mastery demo: {len(demo)} actions')

    print(f'[bc-circ16-mp] collecting {args.reps} reps ...')
    all_obs, all_acts = [], []
    ...
    print(f'[bc-circ16-mp] total: {len(demo_obs)} pairs')
    ...
    print(f'[bc-circ16-mp] training {args.epochs} epochs ...')
    for epoch in range(args.epochs):
        ...
        if epoch == 0 or (epoch + 1) % 50 == 0:
            print(f'[bc-circ16-mp] epoch {epoch+1}/{args.epochs}  loss = {loss.item():.4f}')

    print(f'[bc-circ16-mp] saving {save_path}')
